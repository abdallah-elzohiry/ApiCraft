...
export class TypeScriptGenerator implements CodeGenerator {
    async generate(
        ...
        for (const model of document.models) {
            ...
            await writeFile(
                path.join(modelsDirectory, `${fileName}.ts`),
                content,
                "utf8"
            );
        }
    ...
    private generateModel(model: CodeXaDocument["models"][number]): string {
...
`;
    ...
    private generateIndex(document: CodeXaDocument): string {
