    ...
    .action(async (options) => {
      ...
      for (const endpoint of document.endpoints) {
        ...
        for (const parameter of endpoint.parameters) {
          console.log(
            `${parameter.name}: ${parameter.type} (${parameter.location})`
          );
        }

        if (endpoint.requestBody) {
          console.log(
            `         Request: ${endpoint.requestBody.type}${endpoint.requestBody.required
              ? " (required)"
              : ""
            }`
          );
        ...
        if (endpoint.response) {
          console.log(
            `Response: ${endpoint.response.statusCode} → ${endpoint.response.type}`
          );
        }
      }
      ...
      console.log();
      console.log(
        `✅ Generated successfully: ${options.output}`
      );
    });
}